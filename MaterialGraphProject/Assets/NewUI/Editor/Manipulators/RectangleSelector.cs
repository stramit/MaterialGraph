			...
			switch (evt.type)
			{
				...
				case EventType.MouseUp:
					if (CanStopManipulation(evt))
					{
						...
						selectionRect.min = new Vector2(Math.Min(m_Rectangle.m_Start.x, m_Rectangle.m_End.x), Math.Min(m_Rectangle.m_Start.y, m_Rectangle.m_End.y));
						selectionRect.max = new Vector2(Math.Max(m_Rectangle.m_Start.x, m_Rectangle.m_End.x), Math.Max(m_Rectangle.m_Start.y, m_Rectangle.m_End.y));
